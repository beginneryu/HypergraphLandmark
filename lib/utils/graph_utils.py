...
def incidence_matrix_from_num_points(inc_dir, num_points):
    connection = np.loadtxt(inc_dir)

    return inc_mx_from_list(connection, num_points)
...
def inc_mx_from_list(connection, num_points, is_weight=False):
    if is_weight:
        print("=> using weighted graph")
        weights = connection[:, 2]
    else:
        weights = None
    edges = connection[:, :2]
    return inc_mx_from_weighted_edges(num_points, edges, weights, sparse=False)
...
def inc_mx_from_weighted_edges(num_pts, edges, weights, sparse=True):
    edges = np.array(edges, dtype=np.int32)
    if weights is None:
        weights = np.ones(edges.shape[0])
    data, i, j = weights, edges[:, 0], edges[:, 1]
