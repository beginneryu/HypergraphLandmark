...
from lib.model.STCNet.gcn_model.stgcn import STHGCN
from lib.utils.graph_utils import incidence_matrix_from_num_points, adj_matrix_from_num_points
...
class BHSTPredictor(nn.Module):
    def __init__(self, in_channels, num_points, feat_size, **kwargs) -> None:
        super(BHSTPredictor, self).__init__()
        print("Creating BHSTPredictor ......")
        self.config = kwargs['config']
        self.map_to_node = BaseMapToNode(in_channels=in_channels, num_points=self.config.num_kpts)
        incidence_matrix = incidence_matrix_from_num_points(inc_dir=self.config.adj_dir, num_points=self.config.num_kpts)


        self.tsemhgcn = STHGCN(incidence_matrix, feat_size*feat_size*4, self.config.gru_param.hidden_dim)
    ...
    def forward(self, x):
        ...
        # tsemgcn predicting
        outputs = self.tsemhgcn(tsemgcn_input)
